...
- `AntRecord::OntologyRevision { data: Box<OntologyRevision> }`, counted
  in the trailer as `ontologyRevisions` (`Counts::ontology_revisions`,
  default zero).
- `FORMAT_VERSION` / `SUPPORTED_FORMAT_VERSION` "0.7", `FORMAT_MINOR` 7.
  Same-major policy unchanged: older 0.x archives still read.
- The native payload preserves the complete reviewed manifest, semantic
  and publication closure, approval binding, first publisher and
  idempotency identity, commit time, and `ontology/v1` conditional-head
  position.
- Deterministic `ontology_revisions.ant` conformance generation and
  semantic runner assertions; JSON Schema and all goldens regenerate in
  the coordinated engine build window. Delta note:
  `openantares/deltas/ant-v0.7-delta.md`.
